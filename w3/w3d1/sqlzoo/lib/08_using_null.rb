...
def teachers_and_mobiles
  ...
  execute(<<-SQL)
    SELECT
      name, COALESCE(mobile, '07986 444 2266') AS mobile_number
    FROM
      teachers
  SQL
end
...
def teachers_and_depts
  ...
  execute(<<-SQL)
    SELECT
      teachers.name, COALESCE(depts.name, 'None') AS mobile_number
    FROM
      teachers
      LEFT JOIN
      (
...
def dept_staff_counts
  ...
  execute(<<-SQL)
    SELECT
      depts.name, COALESCE(COUNT(teachers.name), 0) AS num_staff
    FROM
      depts
      LEFT OUTER JOIN
      (
