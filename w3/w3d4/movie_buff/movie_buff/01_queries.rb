...
def harrison_ford
  ...
  Movie.distinct.select(:id, :title).joins(:actors).joins(:castings).where("actors.name = 'Harrison Ford' and castings.ord <> 1")
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:castings).group(:id).order("count(castings.actor_id) DESC").limit(3)
end
